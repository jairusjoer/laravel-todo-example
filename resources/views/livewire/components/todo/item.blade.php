...
<div class="flex gap-10 border rounded-xl p-2 leading-4 text-foreground" x-data="todo">

    <button x-ref="output" class="p-2 text-left truncate" x-on:click="focus" x-show="!edit" title=" Click to edit: {{ $todo->text }}">
        {{ $todo->text }}
    </button>
    ...
    <form class="grow flex gap-1 items-center" wire:submit="update('text')" x-on:submit="blur" x-cloak x-show="edit"
        ...
        @csrf
        <input x-ref="input" wire:model="text" class="grow h-8 p-2 rounded outline-brand bg-background truncate" type="text"
            placeholder="{{ $todo->text }}">
        <button class="button-button-solid" title="Update"><livewire:components.icon variant="pencil" /></button>
    </form>
